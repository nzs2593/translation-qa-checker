...
function exportPdf(issues) {
  ...
  const report = `<!doctype html><html lang="${currentLocale}"><head><meta charset="utf-8"><title>${escapeHtml(currentLocale === "ru" ? "Отчёт CHECKform" : "CHECKform Report")}</title><style>
  ...
  </style></head><body><h1>${escapeHtml(currentLocale === "ru" ? "Отчёт CHECKform" : "CHECKform Report")}</h1><p class="meta">${escapeHtml(sourceLabel)} → ${escapeHtml(targetLabel)} · ${escapeHtml(findingCount(includedIssues.length))}</p><h2>${escapeHtml(currentLocale === "ru" ? "Текст перевода с отмеченными ошибками" : "Target text with marked findings")}</h2><div class="text">${highlightedTextHtml(targetText.value, includedIssues)}</div><h2>${escapeHtml(currentLocale === "ru" ? "Ошибки" : "Findings")}</h2><ol>${issueItems}</ol><footer>${escapeHtml(currentLocale === "ru" ? "Создано CHECKform · Ошибки только отмечены, текст не исправлялся автоматически." : "Generated by CHECKform · Findings are reported only; target text was not auto-corrected.")}</footer></body></html>`;
